...
im .idl - Format einzulesen und daraus Java-Code zu generieren. 
Zum Einen wird eine Abstrakte Klasse Java generiert.
Diese wird auf Serverseite eingebunden und zu einer konkreten Klasse abgeleitet. 
Darüber hinaus soll der Compiler eine Proxy - Klasse für die Clientseite generieren,
welche die Methodenaufrufe über vordefinierte Schnittstellen an den ORB weiterleitet.
...
\begin{itemize}
	...
	\item args[1]: Pfad zur Ausgabedatei der abstrakten Java - Klasse
	\item args[2]: Pfad zur Ausgabedatei der Java Proxy - Klasse
\end{itemize}

\subsubsection{Entwurfsentscheidungen}
Die Methoden zur Übersetzung der IDL - Beschreibung werden gemäß des Adapter - Patterns
in einem Interface definiert. Die Eigentliche Übersetzung nimmt IDLToJavaTranslator und
der IDLToProxyTranslator (die Implementierung des 
Interfaces) vor. So ist der Übersetzungsprozess logisch von der Syntax der zu übersetzenden
Sprache getrennt, und die Testbarkeit der einzelnen Vorgänge gegeben, außerdem können weitere
zu generierende Klassendefinitionen einfach hinzugefügt werden.
\subsubsection{Konfigurationsparameter}
