...
start_reading(Terminated, Logfile, ReaderNNrs, ServerPID):
    (Boolean X Atom X Liste X PID) -> Atom
\end{lstlisting}
...
\subsubsection{Aufgabe und Verantwortung}
Das Editor-Modul übernimmt das Senden von Nachrichten an den Server.

\subsubsection{Schnittstelle}
Der Editor bietet zwei Schnittstellen für den Client: starten und terminieren.


\subsubsection{Entwurfsentscheidungen}
Beim Start des Moduls wird automatisch mit dem Senden von Nachrichten begonnen: Zunächst wird
über die Schnittstelle des Servers die nächste Nachrichtennummer geholt, und danach eine Nachricht mit dieser
Nummer an den Server geschickt. Diese Funktion ruft sich 5 mal rekursiv auf. Bei Terminierung des Vorgangs wird
einmal eine Nachrichtennummer angefragt, ohne eine entsprechende Nachricht zu senden.
Beim Senden einer Nachricht wird mit erlang:now/0 ein Zeitstempel in die Nachricht eingefügt.

Der Reader terminiert wenn: 
\begin{itemize}
    \item Er 5 Nachrichten gesendet hat.
