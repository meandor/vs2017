...
erwartet werden. Erwartete Nachrichten werden weitergeleitet, und es wird Fehlerbehandlung
bei Überfüllung vorgenommen.

\subsubsection{Schnittstelle}

{request,initHBQ}: \newline 
Ruft den receive - Loop der HBQ mit leerer Nachrichtenliste und
					der Initialisierung der DLQ auf. Dieser Aufruf ist zwingend nötig, 
					bevor weitere RPCs ausgeführt werden.

{request,pushHBQ,[NNr,Msg,TSclientout]}:\newline 
Nachricht zwischenspeichen, oder an DLQ weiterleiten. Je nach dem,
ob sie von der DLQ erwartet wird.

{request,deliverMSG,NNr,ToClient}: \newline 
Auftrag der Zustellung wird an DLQ weitergereicht 
und die tatsächlich gesendete NNr zurückgeschickt.

{request,dellHBQ}:\newline 
terminiert die HBQ bedingungslos.
...
Fehlerfall: HBQGröße >= (2/3) * Maximale DLQ Größe  \newline
...
die DLQ gepusht. Danach wird die HBQ wird auf weitere Nachrichten geprüft die nun gesendet werden können.
...
Die Liste wird nach jedem Einfügen anhand der NNr Sortiert, um die Reihenfolge zu erhalten. 
Dies ist nötig, um die erste Lücke in Sequenznummern zu finden. Außerdem ist so das Senden der
ältesten Nachricht performant, O(1).
